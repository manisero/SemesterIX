...
namespace GRM.Logic.UnitTests.GRMAlgorithm.TransactionIDsStorage.StorageStrategies.DiffSetsStorageStrategy
{
    public class SetTreeRootDecisivenessTests
    {
        ...
        public void sets_DecisionTransactionIDs()
        {
            ...
            Assert.Equal(2, root.DecisionsTransactionIDs[1].Support);
            Assert.Equal(new List<int> { 1, 3 }, root.DecisionsTransactionIDs[1].TransactionIDs);

            Assert.True(root.DecisionsTransactionIDs.ContainsKey(2));
            Assert.Equal(1, root.DecisionsTransactionIDs[2].Support);
            Assert.Equal(new List<int> { 2 }, root.DecisionsTransactionIDs[2].TransactionIDs);
        }
    }
}
